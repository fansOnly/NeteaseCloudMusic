...
<view class="pl-desc">
  ...
  <view class="pl-section">简介：{{plData.description}}</view>
</view>
<!-- 歌单歌曲列表 -->
<view class="pl-playlist">
  <view class="pl-bar-title">歌曲列表</view>
  <view class="pl-songs">
    <template v-for="(item, index) in plData.tracks" :key="item.id">
      <view class="pl-songs-item" @tap="handlePlay(item.id)">
        <view class="pl-song-sort">{{index+1}}</view>
        <view class="pl-songs-info">
          <view class="pl-songs-name">{{item.name}}</view>
          <view class="pl-songs-desc">
            <view class="pl-songs-artist">{{item.ar[0].name}}</view>
            <view v-if="item.al.name" class="pl-songs-album">- {{item.al.name}}</view>
          </view>
        </view>
        <view class="pl-songs-more"><view class="at-icon at-icon-chevron-right"></view></view>
      </view>
    </template>
  </view>
</view>
